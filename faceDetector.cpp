...
void FaceDetector::loadFiles(cv::String facecascade_filename, cv::String eyecascade_filename){

    if(!faceCascade.load(faceCascadeFilename)){
    std::cout << "Error Loading" <<faceCascadeFilename << std:: endl;
    }

    if(!faceCascade.load(eyeCascadeFilename)){
...
void FaceDetector::process(cv::Mat frame){
    ...
    faceCascade.detectMultiScale(grey_image, faces, 1.1,2, 0|CV_HAAR_SCALE_IMAGE, cv::Size(frame.cols/4,frame.rows/4));
    ...
    for(size_t i =0; i<faces,size(); i++){
        ...
        for(size_t j=0; j<eyes.size(); j++){
            cv::Point center(faces[i].x + eyes[j].x + eyes[j].width*0.5, face[i].y + eyes[j].y + eyes[j].height*0.5);
